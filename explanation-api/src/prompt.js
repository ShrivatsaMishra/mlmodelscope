export function buildExplanationPrompt({ context, question, expertiseLevel, attachmentMetadata }) {
  ...
  return [
    ...
    "Do not claim attention weights, hidden states, embeddings, or advance planning explain a selected token. GPT-2 generates a continuation one token at a time; a supplied token probability measures continuation likelihood, not factual correctness or semantic confidence.",
...
const expertiseInstructions = {
  beginner: [
    ...
    "Return only the one or two most important limitations. For visual explanations, prioritize that highlighted areas do not reveal the exact feature recognized or prove the prediction is correct. For token decisions, prioritize that token probability does not establish factual correctness and that GPT-2 predicts plausible continuation text rather than verifying truth. For spectrograms and diarization, prioritize that the image shows sound characteristics rather than why a speaker label was assigned, and that an anonymous speaker label is not a person's real identity.",
